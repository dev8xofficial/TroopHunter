...
import { IBusinessCreateRequestAttributes, IBusinessAttributes, IOpeningHourAttributes, IClosingHourAttributes, IDayAttributes, IBusinessDayAttributes } from '@repo/validator';
import { IBusinessRatingAttributes } from '@repo/validator';
import { IBusinessSourceAttributes } from '@repo/validator';
import { IBusinessOpeningHourAttributes } from '@repo/validator';
import { IBusinessClosingHourAttributes } from '@repo/validator';
import { IPostalCodeAttributes } from '@repo/validator';
import { IBusinessPhoneAttributes } from '@repo/validator';
import { ITimezoneAttributes } from '@repo/validator';
...
export const createBusiness = async (req: Request, res: Response) => {
  ...
  if (days?.length)
    days.map((day: IDayAttributes) => {
      daysAndIds[day.day] = { id: day.id };
    });

  if (openingHours?.length)
    openingHours.map((openingHour: IOpeningHourAttributes) => {
      openingHoursAndIds[openingHour.time] = { id: openingHour.id };
    });

  if (closingHours?.length)
    closingHours.map((closingHour: IClosingHourAttributes) => {
