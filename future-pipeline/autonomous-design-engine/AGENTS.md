...
6. **[`END_GOAL.md`](./END_GOAL.md)** — the ratified, versioned target state of the system (Bounded Goal and North Star).
7. **[`investigations/END_GOAL_FEASIBILITY_CONTRACT.md`](./investigations/END_GOAL_FEASIBILITY_CONTRACT.md)** - the ratified feasibility findings (CF/CA series); and **[`CONTRACT_EXECUTION_PLAN.md`](./CONTRACT_EXECUTION_PLAN.md)** - the plan executing them. `investigations/ARCHITECTURE_INVESTIGATION.md` is the investigation record behind them. Research-engine disposition: superseded by the commissioned-investigation pattern in `investigations/` and `knowledge/research-method.md`.

## Rules that override defaults (do not skip this section)

- **Meta-Work Moratorium (D4):** No *new* spec/meta documents may be created until the H1 gate has data. R/M-chunk edits to existing documents are exempt, but any entirely new investigations require an explicit owner commission.
