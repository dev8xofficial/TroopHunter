...
| [`reconciliation-status.md`](./reconciliation-status.md) | **Living progress tracker** for the plan-vs-code reconciliation effort: which `IMPLEMENTATION_PLAN.md` chunks are verified DONE vs. still PARTIAL/MISSING, per-phase detail on what was actually fixed and why, and the exact next steps to resume. | You're picking up reconciliation/build work on ADE — **read this FIRST**, before starting any new phase/chunk work, to avoid re-auditing what's already verified or missing what's still open |
