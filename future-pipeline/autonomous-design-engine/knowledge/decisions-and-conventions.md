...
### C2.2: Vector-store ANN migration path (pgvector)
- **Current state (unchanged for now)**: flat-file cosine store (`library.ts`) — every `searchLibrary()` call is O(n) over the Library's entries, scanning and cosine-comparing each candidate vector in-process. Correct and simple; the right choice while the Library is small (solo-scale, hundreds to low-thousands of entries).
- **Migration trigger**: move off the flat-file store when EITHER (a) `readLibrary().length` measurably exceeds ~5,000 entries (the point flat-file O(n) cosine scan starts costing more wall-clock than a single generation call is worth), OR (b) `searchLibrary()`'s own measured latency (add a timer around it if this becomes a live question) regularly exceeds ~200ms — whichever trips first.
- **Target technology**: PostgreSQL + `pgvector`, using an HNSW index (`ef_construction`/`m` tuned per the pgvector docs, not left at defaults) over the same `intent + context_fit` embedding text this codebase already keeps embed-vs-payload split for (C2.1) — the split is what makes this migration mechanical: only the embedded-text column needs to move into a vector column, `construction`/`recipe_values`/hex/tokens stay exactly where they are as ordinary payload columns.
- **Recall target**: ≥0.95 recall@10 against the flat-file cosine store's exact top-10 on a fixed snapshot (use the per-run Library-version snapshot mechanism below as the fixed reference set) — i.e. the ANN index must agree with exact cosine on at least 19/20 of the top-10 neighbors it returns, verified once at migration time and re-verified after any index-parameter change. Below that, tune `ef_search` up before shipping; this is a correctness bar, not a nice-to-have, because C2.3's similarity floor (`MIN_SIMILARITY`) already discards low-confidence hits — an ANN index that silently drops different entries than exact cosine would would change WHICH advisory pattern surfaces, not just how fast.
- **Migration is NOT scheduled**: this is the specified path for **when** the trigger above fires, not a roadmap item with a date. Building it now, at solo/hundreds-of-entries scale, would be exactly the premature complexity spec/00's own philosophy warns against.
