...
A standalone **meta-capability** built this session at `future-pipeline/autonomous-design-engine/research-engine/`. Not part of ADE's runtime; its job is continuous, evidence-driven **architectural evolution** of ADE. It **proposes; a human ratifies; only then does ADE change** — it never edits `spec/` or `IMPLEMENTATION_PLAN.md` as an accepted change on its own. Full detail lives in `research-engine/` (canonical); this is the distillation.
