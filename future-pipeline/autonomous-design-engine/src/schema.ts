...
// C1.1: semantic/state colors — a brand schema that only carries decorative
// roles (accent/background/text) can't express "this is an error" without
// inventing an off-system color at generation time. Concrete values only
// (04 §3's "concrete, not adjectives"), same as the rest of BrandData.
export const SemanticColorEntrySchema = z.object({
  role: z.enum(['error', 'success', 'warning', 'info']),
  value: z.string().regex(/^#[0-9a-fA-F]{3,8}$/, 'Must be a valid hex color'),
});
export type SemanticColorEntry = z.infer<typeof SemanticColorEntrySchema>;

// C1.1: theming/dark-mode axis. `enabled: false` (the default) means the
// brand is light-only — still schema-complete, just declaring the axis is
// off, rather than the axis not existing at all.
export const DarkModeAxisSchema = z.object({
  enabled: z.boolean().default(false),
  palette: z.array(PaletteEntrySchema).optional(), // dark-variant overrides, same roles as the light palette
  semanticColors: z.array(SemanticColorEntrySchema).optional(),
});
export type DarkModeAxis = z.infer<typeof DarkModeAxisSchema>;
...
export const BrandDataSchema = z.object({
  ...
  semanticColors: z.array(SemanticColorEntrySchema).optional(), // C1.1 — error/success/warning/info
  darkMode: DarkModeAxisSchema.default({ enabled: false }), // C1.1
...
export const AutonomyRungSchema = z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3), z.literal(4)]);
...
export const DimensionScoresSchema = z.object({
  ...
  reward_model_adjustment: z.number().optional(), // C3.5 — how much the RM altered the score
});
...
export const CandidateScoreSchema = z.object({
  ...
  reward_model_signals: z
    .object({
      universal_craft: z.number(),
      domain_style: z.number(),
    })
    .optional(), // C3.5 — separated signals from the dual-judge RM
...
export const CriticOutputSchema = z.object({
  ...
  position_maps: z.array(z.record(z.string(), z.number())).optional(), // Audit log of randomized orders
});
...
export const LibraryEntryTypeSchema = z.enum(['principle', 'pattern', 'component-recipe', 'anti-pattern']);
...
export const BrandIdentitySchema = z.object({
  ...
  semanticColors: z.array(SemanticColorEntrySchema).default([]), // C1.1 — carried through from BrandData, never dropped
  darkMode: DarkModeAxisSchema.default({ enabled: false }), // C1.1
  logo_ref: z.string().optional(),
});
...
// C1.2: a single derived candidate among the 2-3 the derivation pipeline
// must produce — the identity (personality/tone/motion/usage) PLUS the
// rationale tying it to the business context, so a direction is never just
// "a variant" but a justified strategic choice a human/critic can compare
// against its siblings.
export const BrandDirectionSchema = z.object({
  label: z.string().min(1), // short name, e.g. "Confident Minimalist"
  identity: BrandIdentitySchema,
  rationale: z.string().min(1), // WHY this direction fits the business context
});
...
// C1.1: fluid scale entry — the standard clamp() triple (min at the
// narrowest supported viewport, max at the widest, preferred as a
// viewport-relative value in between). Additive alongside the existing
// fixed-px maps below — a token can have a fixed value, a fluid value, or
// both; nothing existing changes shape.
export const FluidScaleEntrySchema = z.object({
  min: z.string().min(1), // e.g. "32px" at the narrowest breakpoint
  max: z.string().min(1), // e.g. "64px" at the widest breakpoint
  preferred: z.string().min(1), // e.g. "5vw" — the viewport-relative middle term
});
export type FluidScaleEntry = z.infer<typeof FluidScaleEntrySchema>;

// C1.1: standard token export format (DTCG / Style-Dictionary shape) —
// present even if initially unused, per the plan, so a downstream export
// pipeline has a real target to consume rather than a schema surprise.
export const TokenExportFormatSchema = z.enum(['dtcg', 'style-dictionary']);
export type TokenExportFormat = z.infer<typeof TokenExportFormatSchema>;
...
export const DesignTokensSchema = z.object({
  ...
  fluidType: z.record(z.string(), FluidScaleEntrySchema).default({}), // C1.1 — e.g. "display": clamp() triple
  fluidSpace: z.record(z.string(), FluidScaleEntrySchema).default({}), // C1.1 — e.g. "section": clamp() triple
  exportFormat: TokenExportFormatSchema.default('dtcg'), // C1.1
});
...
// C1.7: an additive, namespaced token extension — a genuinely-needed new
// token added without altering any existing one. Namespaced under
// "ext-" so it can never collide with (or be silently mistaken for) a
// foundation token, and every extension is logged so frequency is a real,
// queryable metric instead of a hardcoded mock value.
export const TokenExtensionSchema = z.object({
  category: z.enum(['color', 'type', 'space', 'radius', 'shadow', 'motion']),
  key: z.string().min(1), // stored under "ext-<key>" in tokens.<category>
  value: z.string().min(1),
  reason: z.string().min(1), // why the frozen foundation couldn't express this
  addedBySection: z.string().min(1),
...
export const ProjectDesignSystemSchema = z.object({
  ...
  extensionLog: z.array(TokenExtensionSchema).default([]), // C1.7 — real frequency tracking
});
...
export const ArtifactQAReportSchema = z.object({
  ...
  variation_score: z.number().min(0).max(1).default(1), // C1.10 — structural diversity across sections; 1 = distinct-purpose sections all differ, 0 = every section structurally identical
  summary: z.string(),
  violations: z.array(ViolationSchema),
});
...
export const InputBundleSchema = z.object({
  ...
  ctxShots: z
    .array(
      z.object({
        // screenshots of already-built sections
...
export const VerdictEntrySchema = z.object({
  ...
  annotations: z
    .array(
      z.object({
        ...
        y: z.number(),
        text: z.string(),
        target: z.enum(['iter0', 'final']),
      }),
    )
    .optional(),
  rationale: z.string().optional(),
  r16_lite_outcome: z.enum(['shipped', 'abandoned', 'reworked']).optional(),
  ab_test_variant: z.enum(['text_only', 'visual_annotated']).optional(),

  // C3.2 Phase 3 Autonomy ladder fields
  stratum: z.enum(['routine', 'hard', 'adversarial']).optional(),
  ...
  review_route: z.enum(['full-review', 'spot-check', 'audit-only']).optional(),
});

export type VerdictEntry = z.infer<typeof VerdictEntrySchema>;

// ─── Retest baseline (C0.16 / E0.7 — quarterly human test-retest ritual) ──

// A single held-out case in the frozen retest set — the SAME artifact
// re-presented blind later; the reviewer field is captured so cross-rater
// vs self-rater agreement can eventually be told apart, not just merged.
export const RetestCaseSchema = z.object({
  case_id: z.string().min(1),
  run_id: z.string().min(1),
  section: z.string().min(1),
  iter0_shots_dir: z.string().min(1),
  final_shots_dir: z.string().min(1),
  baseline_rating: z.enum(['bad', 'weak', 'good', 'strong']),
  baseline_preferred: z.enum(['iter0', 'final']),
  baseline_reviewer: z.string().optional(),
  baseline_captured_at: z.string(),
});
export type RetestCase = z.infer<typeof RetestCaseSchema>;

export const RetestSetSchema = z.object({
  frozen_at: z.string(),
  cases: z.array(RetestCaseSchema).min(1),
});
export type RetestSet = z.infer<typeof RetestSetSchema>;
...
const SCHEMA_REGISTRY: Record<string, z.ZodType> = {
  ...
  retestSet: RetestSetSchema, // C0.16
  ...
  brandDirections: z.object({ directions: z.array(BrandDirectionSchema).min(2).max(3) }), // C1.2
 ...
 */
export function validate<T = unknown>(schemaName: string, json: unknown): { success: true; data: T } | { success: false; error: string } {
