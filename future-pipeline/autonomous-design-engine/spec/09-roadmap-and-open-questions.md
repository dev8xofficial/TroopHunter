...
5. **Surface representation.** *Resolved in Phase 3 (C3.7):* The initial unknown was how to judge interaction states (empty/loading/error). This is now solved: the **Eyes drive the component through its states** via Playwright (clicking, hovering, typing) and capture a prioritized canonical set of state screenshots for the Critic to judge, rather than relying on a single static frame.
