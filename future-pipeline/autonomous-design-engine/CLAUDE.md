...
Everything else — architecture, build order, conventions, open questions — is in [`AGENTS.md`](./AGENTS.md), [`spec/`](./spec/), [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md), and [`knowledge/`](./knowledge/). `END_GOAL_FEASIBILITY_CONTRACT.md` contains the ratified feasibility findings, executed by `CONTRACT_EXECUTION_PLAN.md` (for the research-engine's disposition, see D1).
