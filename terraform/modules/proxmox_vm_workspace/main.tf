# Workspace VM module — identical to ../proxmox_vm EXCEPT it omits `prevent_destroy`.
# Workspace hosts (e.g. Kasm) are disposable cattle that must be destroy/recreate-able via IaC.
...
resource "proxmox_vm_qemu" "vm" {
  ...
  lifecycle {
    # NOTE: `prevent_destroy` intentionally omitted so workspace VMs can be
    # destroyed and recreated entirely through Terraform.
    ignore_changes = [
      sshkeys,
      network,
    ]
