    ...
    <div className="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
      <div className="lg:flex lg:gap-x-16">
          ...
          <nav className="flex-none px-4 sm:px-6 lg:px-0">
            <ul role="list" className="flex gap-x-3 gap-y-1 whitespace-nowrap lg:flex-col">
              {navigationItems.map((item) => (
                <li key={item.name}>
                  <Link to={item.href} className={classNames(location.pathname === item.href ? 'bg-gray-50 text-indigo-600' : 'text-gray-700 hover:bg-gray-50 hover:text-indigo-600', 'group flex w-full gap-x-3 rounded py-2 pl-2 pr-3 text-sm font-semibold leading-6')}>
                    <item.icon className={classNames(location.pathname === item.href ? 'text-indigo-600' : 'text-gray-400 group-hover:text-indigo-600', 'h-6 w-6 shrink-0')} aria-hidden="true" />
                    {item.name}
                  </Link>
                </li>
              ))}
        ...
        <main className="w-full px-4 py-4 sm:px-6 lg:flex-auto lg:px-0">
          <div className={classNames('w-full mx-auto max-w-2xl lg:mx-0', location.pathname === '/settings/profile' ? 'space-y-16 sm:space-y-20' : '')}>
            {navigationItems.map((item, index) => (
              <div key={index} className={location.pathname === item.href ? 'block' : 'hidden'}>
                <h2 className="text-base font-semibold leading-7 text-gray-900">{item.name}</h2>
