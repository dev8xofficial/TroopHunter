  ...
  return (
    <div className="relative w-full">
      <div className="flex rounded shadow-sm">
        <input id={id} name={name} type={type} style={style} value={inputValue} ref={inputRef} disabled={disabled} required={required} onChange={onChange} placeholder={placeholder} autoComplete={autoComplete} className={classNames('dark:bg-black-900 block w-full rounded-none rounded-l border-0 px-3 py-1.5 outline-none transition duration-200 placeholder:text-gray-400 dark:text-white sm:text-sm', noring ?? false ? '' : 'ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600', disabled ?? false ? 'bg-gray-100 text-gray-400' : '', className)} />
