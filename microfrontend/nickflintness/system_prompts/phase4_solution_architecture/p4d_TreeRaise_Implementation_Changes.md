# TreeRaise — Implementation Change Instructions
> **Purpose:** This document lists every change required in `admin.html` and `partner-portal.html` based on the Tech Spec Verification Report (Tech_Spec_TreeRaise.docx v1.0). Each change is explicit and actionable. Implement all items marked 🔴 first, then 🟡, then ⚪.

---

## Context

- **Spec:** TreeRaise platform, treeraise.com — digital fundraising platform with 50/50 revenue split (org keeps 50%, TreeRaise keeps 50%)
- **Contribution tiers:** Seed $15 = 1 tree · Sapling $45 = 3 trees · Grove $84 = 6 trees · Legacy $156 = 12 trees
- **User roles:** Campaign Organizer · Supporter/Donor · TreeRaiser Coach · Platform Admin · Sustainability Specialist · Super Admin
...
**Rule:** On every contribution, 50% goes to the organization, 50% goes to TreeRaise.

**In `admin.html` — find and replace all instances of:**
- `"TreeRaise Revenue (8%)"` → `"TreeRaise Revenue (50%)"`
- `"Fee Rate: 8%"` → `"Split: 50%"`
- `"TR Fee: 8%"` → `"TR Share: 50%"`
...
- `"8% TreeRaise platform fee"` → `"50% TreeRaise share"`
...
**What to build:** A new sidebar nav item and panel for TreeRaiser Coach management and task queue.
...
2. Add a **"By Sector" breakdown table** below the existing revenue table:
   - Columns: Sector · Organizations · Gross Raised · TreeRaise Share (50%) · Org Payout (50%)
...
- On "Finish & Generate URL" button click: show a success state with a generated campaign URL like `treeraise.com/c/riverside-pta-spring-2026`, a "Copy Link" button, and a "Go to Dashboard →" button (calls `showPage('page-home', ...)`)
...
|---|---|---|---|---|---|
| 1 | Launch Email | Day 0 (on go-live) | Our TreeRaise Fundraiser Starts Today | ✅ Sent Mar 1 | Preview |
| 2 | Mid-Drive | Day 15 | Halfway There — Thank You for Your Support | 🕐 Scheduled Mar 16 | Preview · Edit |
| 3 | Final Push | Day 28–29 | Last Day to Support Our TreeRaise Fundraiser | 🕐 Scheduled Mar 29 | Preview · Edit |
| 4 | Wrap-Up | Within 24h of close | Thank You for Supporting Our TreeRaise Fundraiser | ⏸ Pending campaign close | — |
...
*Based on Tech_Spec_TreeRaise.docx v1.0 · Verified March 14, 2026*
