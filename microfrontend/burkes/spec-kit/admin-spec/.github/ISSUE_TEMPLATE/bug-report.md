...
- type: input
  ...
  placeholder: "e.g., 001-dashboard, 007-partner-referrals"
  validations:
  required: true
...
- type: textarea
  ...
  placeholder: |
  Example:
  The "transaction_status" field in constitution.md lists 6 statuses but the Transactions spec
  (002) uses an additional "archived" status that is not in the canonical list.
  validations:
  required: true
...
- type: textarea
  ...
  placeholder: |
  File: .specify/specs/002-transactions/spec.md
  Section: ## Data & State
  Line: approx. 180

      File: .specify/memory/constitution.md
      Section: ## 5. Transaction Status Labels
...
- type: textarea
  ...
  placeholder: |
  Developers will be confused about valid transaction statuses.
  Implementation may create invalid state combinations.
  Tests may fail because constitution and spec don't match.
  validations:
  required: true
...
- type: textarea
  ...
  placeholder: |
  Example:
  Remove "archived" from Transactions spec, or
  add "archived" to constitution.md Section 5 with an accepted definition.
