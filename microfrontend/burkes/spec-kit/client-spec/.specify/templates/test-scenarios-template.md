...
| Agent    | **Action 1** | State X      | Agent sees Y, system does Z    | ✅ Pass / ❌ Fail |
...
**Example**: Agent and Lender both upload documents at same time  
**Actors**: Agent (thread 1), Lender (thread 2)  
**Precondition**: Both logged in to same transaction

### Steps

1. Agent starts upload (don't wait for completion)
...
```json
{
  ...
  "doc_1": { "filename": "agent-doc.pdf", "uploader": "agent" },
...
```json
{
  ...
  "roles": ["client", "agent", "lender", "attorney", "cpa", "tc"],
  "documents": [
    {
      "id": "doc-001",
