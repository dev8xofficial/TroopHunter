# Tasks Prompt — The Burkes Group Client Portal
...
2. Replace `[PLAN_FILE_PATH]` with the path to the target plan (e.g., `.specify/specs/007-closing-checklist/plan.md`).
...
You are generating developer tasks for **The Burkes Group Client Portal**. Tasks must be written for a frontend developer who is familiar with the portal's existing patterns but may not have deep context on the specific feature.
...
|---------|--------------------------|
| Screen shell (page-body, pg-header) | Any screen in `client.html` |
| Card component with card-hdr / card-bd | Dashboard sidebar cards |
| Badge rendering | Document table status column |
| Alert banner | Dashboard top alert |
| Form input styling | Insurance screen cards |
| Table layout (tbl-wrap > table > th/td) | Documents screen tables |
| Progress bar | Mortgage progress overview card |
| Timeline item | Dashboard transaction progress |
| Thread list + chat panel | Messages screen |
...
Examples:
- `TASK-007-01 — Define checklist data model and state shape`
- `TASK-007-02 — Build ChecklistCard component (FR-07-02)`
- `TASK-007-03 — Implement item completion toggle with activity log write`
