...
export const isHelloabdulPort = (baseUrl?: string): boolean => {
  ...
  if (!url) {
    // Default to false for dev8x project (this file is in dev8x)
    return false;
  }
  ...
  try {
    ...
    // If port doesn't match known ports, default to false for dev8x project
    return false;
  } catch (error) {
    // If URL parsing fails, default to false for dev8x project
    return false;
  }
};
