...
Implement the password reset specification for the authentication & identity domain with contract-first validation, RBAC enforcement, and append-only audit coverage.
...
### Task 1: Model PasswordResetToken

**Complexity**: M
**Priority**: P0

**Description**: Finalize canonical data structures, validation rules, and ownership boundaries for password reset.
...
**Description**: Implement the request and response contracts for POST /api/v1/auth/password/request, POST /api/v1/auth/password/verify-token, POST /api/v1/auth/password/reset.
...
### Task 4: Implement password reset token lifecycle
...
| interactions.yaml | Yes | Publishes Password Reset Token Lifecycle transitions |
...
| --- | --- | --- |
| Phase 1 | 1-2 days | Model PasswordResetToken, Deliver core API surface, Enforce RBAC and data scoping |
| Phase 2 | 2-3 days | Implement password reset token lifecycle, Wire audit events, Add validation and regression coverage |
