# Candidate Messages - Implementation Plan
...
Implement the candidate messages specification for the candidate portal domain with contract-first validation, RBAC enforcement, and append-only audit coverage.
...
### Task 1: Model CandidateThread

**Complexity**: M
**Priority**: P0

**Description**: Finalize canonical data structures, validation rules, and ownership boundaries for candidate messages.
...
**Description**: Implement the request and response contracts for GET /api/v1/candidate/messages, GET /api/v1/candidate/messages/{id}, POST /api/v1/candidate/messages/{id}/reply, POST /api/v1/candidate/messages/{id}/read.
...
### Task 4: Implement candidate thread lifecycle
...
| interactions.yaml | Yes | Publishes Candidate Thread Lifecycle transitions |
...
| --- | --- | --- |
| Phase 1 | 1-2 days | Model CandidateThread, Deliver core API surface, Enforce RBAC and data scoping |
| Phase 2 | 2-3 days | Implement candidate thread lifecycle, Wire audit events, Add validation and regression coverage |
