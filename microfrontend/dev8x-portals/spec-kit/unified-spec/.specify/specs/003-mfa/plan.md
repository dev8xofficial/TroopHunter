...
Implement the mfa specification for the authentication & identity domain with contract-first validation, RBAC enforcement, and append-only audit coverage.
...
### Task 1: Model MfaChallenge

**Complexity**: M
**Priority**: P0

**Description**: Finalize canonical data structures, validation rules, and ownership boundaries for mfa.
...
**Description**: Implement the request and response contracts for POST /api/v1/auth/mfa/challenge, POST /api/v1/auth/mfa/verify, POST /api/v1/auth/mfa/recovery.
...
### Task 4: Implement mfa challenge lifecycle
...
**Description**: Emit 4 append-only events with payloads aligned to contracts/events.yaml.
...
| events.yaml | Yes | Registers 4 append-only audit events |
| interactions.yaml | Yes | Publishes MFA Challenge Lifecycle transitions |
...
| --- | --- | --- |
| Phase 1 | 1-2 days | Model MfaChallenge, Deliver core API surface, Enforce RBAC and data scoping |
| Phase 2 | 2-3 days | Implement mfa challenge lifecycle, Wire audit events, Add validation and regression coverage |
