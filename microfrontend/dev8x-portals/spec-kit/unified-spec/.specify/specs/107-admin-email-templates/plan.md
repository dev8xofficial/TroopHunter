...
Implement the admin email templates specification for the hr admin panel domain with contract-first validation, RBAC enforcement, and append-only audit coverage.
...
### Task 1: Model EmailTemplate

**Complexity**: M
**Priority**: P0

**Description**: Finalize canonical data structures, validation rules, and ownership boundaries for admin email templates.
...
**Description**: Implement the request and response contracts for GET /api/v1/admin/templates, POST /api/v1/admin/templates, POST /api/v1/admin/templates/{id}/preview.
...
### Task 4: Implement template lifecycle
...
| interactions.yaml | Yes | Publishes Template Lifecycle transitions |
...
| --- | --- | --- |
| Phase 1 | 1-2 days | Model EmailTemplate, Deliver core API surface, Enforce RBAC and data scoping |
| Phase 2 | 2-3 days | Implement template lifecycle, Wire audit events, Add validation and regression coverage |
