...
const apiMiddleware: Middleware =
  ...
  async (action: unknown) => {
    // First check if action has the basic structure of a Redux action
    if (typeof action === 'object' && action !== null && 'type' in action) {
      const reduxAction = action as { type: string };

      if (ApiRequestAction.match(reduxAction)) {
        // Type assertion after the match check ensures type safety
        const typedAction = action as PayloadAction<any>;

        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        const { url, method, params, data, onSuccess, headers, requireAuth = false, payload } = typedAction.payload;
        ...
        try {
          ...
          const successPayload = { request: typedAction.payload, response: response.data, payload: payload as Record<any, any> };
          ...
          try {
            ...
            try {
              ...
              const successActionType = onSuccess;
              const successPayload = { request: typedAction.payload, response: retryResponse.data };
              dispatch(refreshTokenSuccessAction({ accessToken: refreshResponse.data.accessToken }));
              dispatch({ type: successActionType, payload: successPayload });
            } catch (refreshError) {
