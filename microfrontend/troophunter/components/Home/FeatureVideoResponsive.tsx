            ...
            <div className="bg-transparent rounded-[clamp(1.25rem,1.3227513228vw,1.6625rem)] text-[inherit] no-underline relative w-full block lg:rounded-[clamp(1.875rem,1.9841269841vw,2.49375rem)]">
              <div className="relative overflow-hidden grid rounded-[inherit] pt-[calc(var(--aspect-y)/var(--aspect-x)*100%)]">
                ...
                <div className="h-auto grid transform -translate-y-2/4 -translate-x-2/4 left-2/4 top-2/4 absolute w-[38.0281690141%] sm:w-[69.4214876033%] sm:items-center">
                  ...
                  <video className="rounded-inherit z-0 aspect-square row-start-1 col-start-1 row-end-auto col-end-auto w-auto object-center object-contain hidden sm:block sm:dark:hidden" src={prefixed('/videos/troophunter-lead-gen/light/feature.mp4')} preload="metadata" loop controls={false} autoPlay muted playsInline></video>
                  <video className="rounded-inherit z-0 aspect-square row-start-1 col-start-1 row-end-auto col-end-auto w-auto object-center object-contain hidden sm:dark:block" src={prefixed('/videos/troophunter-lead-gen/dark/feature.mp4')} preload="metadata" loop controls={false} autoPlay muted playsInline></video>
                  <video className="rounded-inherit z-0 aspect-square row-start-1 col-start-1 row-end-auto col-end-auto h-full scale-[1.75] sm:h-auto sm:scale-[1] object-center object-contain dark:hidden sm:!hidden" src={prefixed('/videos/troophunter-lead-gen/light/feature-mobile.mp4')} preload="metadata" loop controls={false} autoPlay muted playsInline></video>
                  <video className="rounded-inherit z-0 aspect-square row-start-1 col-start-1 row-end-auto col-end-auto h-full scale-[1.75] sm:h-auto sm:scale-[1] object-center object-contain hidden dark:block sm:!hidden" src={prefixed('/videos/troophunter-lead-gen/dark/feature-mobile.mp4')} preload="metadata" loop controls={false} autoPlay muted playsInline></video>
                </div>
              </div>
            </div>
          </div>
