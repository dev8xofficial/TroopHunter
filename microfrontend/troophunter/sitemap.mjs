...
const componentToUrlMap = {
  SignIn: { path: '/signin', priority: 0.5 },
  SignUp: { path: '/signup', priority: 0.5 },
  ForgotPassword: { path: '/forgot-password', priority: 0.4 },
  Home: { path: '/', priority: 1.0 },
};
...
const generateSitemap = async (updatedPaths = new Set(Object.values(componentToUrlMap))) => {
  try {
    ...
    const sitemapStream = new SitemapStream({ hostname: "https://www.troophunter.com" });
