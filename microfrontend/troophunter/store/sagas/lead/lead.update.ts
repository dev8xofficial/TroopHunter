import { type ILeadAttributes, type ApiResponse } from '@repo/validator';
...
function* updateLeadSaga({ payload }: { payload: IUpdateLeadPayload }): Generator<StrictEffect, void, void> {
  try {
    const { id, userId, businessIds, title, search, businessDomain, categoryId, address, cityId, stateId, countryId, postalCodeId, phone, email, website, ratingId, reviews, timezoneId, sponsoredAd, businessCount, accessToken } = payload;

    const apiPayload = {
      url: `/leads/${id}`,
      method: 'PUT',
      params: { accessToken },
      data: removeEmptyStringValues({ userId, businessIds, title, search, businessDomain, categoryId, address, cityId, stateId, countryId, postalCodeId, phone, email, website, ratingId, reviews, timezoneId, sponsoredAd, businessCount }),
...
export interface IUpdateLeadSuccessPayload {
  request: IApiRequestAttributes<IUpdateLeadPayload, IUpdateLeadPayload, undefined, undefined>;
