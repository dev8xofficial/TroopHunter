...
import { type IUserAttributes, type ILeadAttributes } from '@repo/validator';
  ...
  return (
    <>
      ...
      <div className="flex flex-1 flex-col bg-gray-100">
        {/* Filters Head */}
        <div className="dark:divide-charcoal-100 dark:border-charcoal-100 dark:bg-charcoal-500 flex divide-x border-b border-gray-200 bg-white">
          {/* Filters Tab */}
            ...
            <div>
              <div className="flex items-center justify-between px-4">
                <div className="relative flex flex-grow items-center justify-between space-x-6 sm:justify-start">
                      ...
                      <>
                        ...
                        {!isSmScreen && businessesTotalRecords !== null && (
                          <>
                            <Link
                              href=""
                              className="dark:text-secondary-text absolute right-0 top-4 z-10 h-full pr-3 text-xs text-gray-400 outline-none focus:outline-none"
                              onClick={() => {
                                if (!isXLScreen) setIsOpenMobileFiltersDialog(true);
                              }}
                            >
                              {draftLeadBusinessIds !== undefined && draftLeadBusinessIds?.length > 0 ? (
                                <>
                                  {leadPageRemoveSavedBusinesses ? 'Removed' : 'Saved'}:<span className="dark:text-primary-text font-semibold text-gray-500">{draftLeadBusinessIds?.length}</span> |
                                </>
                              ) : null}
                              <span>
                              ...
                              <span>
                                Filtered: <span className="dark:text-primary-text font-semibold text-gray-500">{businessesTotalRecords}</span>
                              </span>
                            </Link>
                          </>
                        )}
                          ...
                          countChangedFilters(leadPageFilters, defaultFilterValues) > 0 && (
                            <span className="absolute -right-2 top-0 block transform">
                              <span className="relative flex h-4 w-4">
                                <span className="text-xxs absolute z-10 inline-flex h-full w-full items-center justify-center rounded-full bg-indigo-500 text-white">{countChangedFilters(leadPageFilters, defaultFilterValues)}</span>
                  ...
                  <Link href="" className="hidden items-center whitespace-nowrap text-sm font-semibold">
                    Saved searches
                  </Link>
                </div>
        ...
        <div className="dark:divide-charcoal-100 flex flex-1 flex-row justify-between divide-x">
          ...
          <div className={classNames(filtersPanelWidth ? 'xl:max-w-lg 2xl:max-w-xl' : '', 'relative hidden w-full max-w-sm bg-white xl:block', 'transition-all duration-500 ease-in-out')}>
            <div className="dark:border-charcoal-100 dark:bg-charcoal-500 flex w-full items-center justify-between border-b bg-white px-4 py-4 text-sm shadow">
              <label htmlFor="selectAll" className="leading-6 text-gray-900 dark:text-white">
                0 filters applied
              </label>
            </div>
            <div className="dark:bg-charcoal-300 hidden overflow-y-scroll p-4 pb-24 xl:col-span-4 xl:block" ref={mainRef} style={{ height: mainHeight }}>
              <div>
                <ul className="dark:divide-charcoal-100 dark:border-charcoal-100 dark:bg-charcoal-500 divide-y rounded border bg-gray-100 shadow">
            ...
            {/* Action buttons */}
            <div className={classNames(filtersPanelWidth ? 'xl:max-w-lg 2xl:max-w-xl' : '', 'dark:border-charcoal-100 dark:bg-charcoal-300 fixed bottom-0 w-full max-w-sm flex-shrink-0 border-t   border-gray-200 bg-white px-4 py-5 sm:px-6', 'transition-all duration-500 ease-in-out')}>
              <div className="flex items-center justify-between">
                ...
                <div className="ml-auto flex justify-end space-x-3">
                  ...
                  {leadPageDraftLeadId.length > 0 && !compareFiltersAndLead(leadPageFilters, draftLead) && (
                    <Link href="" className="inline-flex cursor-pointer items-center text-sm font-semibold" onClick={() => dispatch(restoreHomePageFiltersAction(draftLead))}>
                      Restore
                    </Link>
                  )}
                  <>
...
Lead.getLayout = (page: React.ReactElement) => {
