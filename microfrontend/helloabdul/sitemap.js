...
// Define a record to map page names to URLs for helloabdul
const pageToUrlMap = {
  // Main pages
  index: '/',
  'work/index': '/work',
...
const generateSitemap = async (updatedPaths = new Set(Object.values(pageToUrlMap))) => {
  try {
    console.log('Generating sitemap for helloabdul...');
    ...
    if (newSitemapContent !== existingSitemapContent) {
      ...
      console.log('helloabdul sitemap updated successfully.');
    } else {
      console.log('helloabdul sitemap content unchanged.');
    }
  } catch (error) {
    console.error('Error generating helloabdul sitemap:', error);
  }
};
...
const startFileWatcher = () => {
  ...
  console.log('File watcher started. Monitoring for changes in helloabdul page files and data files...');
};
...
if (isProduction) {
  console.log('Starting helloabdul sitemap generation in production mode...');
  generateSitemap().catch(console.error);
  startFileWatcher();
} else {
  console.log('Generating helloabdul sitemap in development mode for testing...');
  generateSitemap().catch(console.error);
}
