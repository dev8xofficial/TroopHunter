...
def setup_logger(city_name, stateCode, countryCode, lat, long, queue):
    ...
    global logger  # Reference the global logger
...
def fetch_google_places(lat, long, place_type, page_token=None, timeout=30):
    ...
    try:
        logging.info("Starting nearby search for location (%s, %s) with type '%s'.", lat, long, place_type)
        ...
        logging.info("Received response from Nearby Search API with status: %s", status)

        if status == "OK":
            logging.info("Successfully fetched %d results.", len(places_data.get("results", [])))
            # Process results
            for place in places_data.get("results", []):
                place_id = place.get("place_id")

                if place_id:
                    logging.info("Fetching details for place_id: %s", place_id)
                    ...
                    try:
                        ...
                        if details_status == "OK":
                            logging.info("Successfully fetched details for place_id: %s", place_id)
                            ...
                            logging.warning(f"Skipping place_id {place_id} due to status: {details_status}.")
                            continue  # Skip this place and move to the next
                        elif details_status == "OVER_QUERY_LIMIT":
                            logging.error("Over query limit while fetching details for place_id: %s. Shutting down the scraper.", place_id)
                            raise Exception("Over query limit.")

                    except requests.exceptions.Timeout:
                        logging.error("Details request for place_id %s timed out.", place_id)
                    except requests.exceptions.RequestException as e:
                        logging.error("Error fetching details for place_id %s: %s", place_id, e)

            # Return the enriched places data
            logging.info("Enriched places data ready to return.")
            return places_data

        elif status == "ZERO_RESULTS":
            logging.info("No results found for the specified location and type.")
            ...
            logging.error(f"Invalid request for nearby search: {status}. Skipping this queue.")
            return None  # Skip this queue by returning None

        elif status == "OVER_QUERY_LIMIT":
            logging.error("Over query limit for nearby search. Shutting down the scraper.")
            raise Exception("Over query limit.")  # Raise an exception to shut down the scraper

    except requests.exceptions.Timeout:
        logging.error("The nearby search request timed out. Please try again later.")
    except requests.exceptions.RequestException as e:
...
def save_data_to_file(city_name, stateCode, countryCode, lat, long, new_data, queue):
...
def process_queue(queue, city, business_source_id):
    ...
    setup_logger(city_name, city_stateCode, city_countryCode, lat, long, queue)

    while True:
        try:
            # Fetch the first set of places from the API
            response = fetch_google_places(lat, long, queue['searchQuery'], page_token=next_page_token)

            if response is None:
                break  # Skip this queue

            # Save or merge the response data
            save_data_to_file(city_name, city_stateCode, city_countryCode, lat, long, response, queue)
            ...
            while True:
                if is_internet_available():
                    logging.error("Internet connection is working. Retrying...: {e}")
                    break
                else:
                    logging.error("An error occurred while processing the queue due to unknow reason: {e}")
