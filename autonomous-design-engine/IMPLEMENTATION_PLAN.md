...
3. **The 12 invariants (`11 §7`) are a test contract**, not prose. Every invariant has an assertion in the test suite (Appendix B). Code that violates an invariant is a bug regardless of behavior.
...
│  ├─ prompts.ts                 # generator/critic/(later)crystallizer builders [05 §6]
...
│  ├─ critic.ts                  # critique(shots, bundle) → scores (vision, fresh ctx)
...
**Spec:** `06 §2–4` · `04 §2.1, §3` · `03 §3,§4,§8` · `11 §5`. **Proves:** H4 (zero token drift, variety kept) + sets up H5.
...
### Deliverables
`store.ts` (atomic/versioned hard stores); brand derivation + approval; `crystallizer`; multi-section orchestration; token-allowlist gate; new CLI subcommands `design brand` / `design section` / `design site`; whole-artifact QA.
...
- **`brand.ts` (new):** `deriveBrand(brandData, brief) → BrandFoundation(draft)` (palette/type verbatim, personality/tone/motion **derived**, provenance set); **brand-palette a11y pre-check at approval** — run a deterministic contrast check on the provided palette pairings *before* freeze, because a pale-on-white palette would doom every downstream a11y gate (F-BRD-04, High); block approval until accessible primary pairings exist. `approveBrand` → status frozen, version++; `reDerive` on a changed given → new version (never hand-patch a derived leaf — `04 §2.1`).
- **`prompts.ts` +** derive-brand prompt + **crystallizer** prompt (`05 §6.3`).
- **`crystallizer.ts` (new):** after section-1 approval, extract `ProjectDesignSystem` (tokens as Tailwind theme + CSS vars; component recipes the hero used); freeze foundation; lock hero components (`04 §3`).
...
- **Testing:** **de-id gate** blocks client name/tokens/copy (no identity leak); retrieval relevance on seeded entries; **H6 A/B** — same briefs Library-on vs off, compare trace scores.
...
- **Architecture / key modules:** `verdicts.ts` (capture human approve/reject/notes → durable store, fed to calibration); pairwise-ranking refinement in `critic.ts`; `calibration.ts` (track the **Critic-vs-human agreement gap** — I12; tune threshold/weights; rotate rubric examples); wire **autonomy ladder rung 0→1** (`09 §2`).
...
I1 precedence (assembleInputBundle) · I2 critic fresh-context (no generator history in critic call) · I3 objective→Guardrail not Critic · I4 best-so-far never regresses · I5 hard stores atomic/versioned/deliberate-events (P1) · I6 trace persisted before next iteration · I7 Library via de-id gate (P2) · I8 refs soft/≤5/never resemblance-scored · I9 brief-as-data injection safety · I10 one terminal state per run · I11 render-valid precedes critique · I12 quality human-anchored (P3). Each = one assertion in `tests/`.

## Out of scope / explicit non-goals
- No monorepo cross-imports; ADE stays standalone.
- No vector DB, Brand store, or Design System in Phase 0 (`07` scope).
