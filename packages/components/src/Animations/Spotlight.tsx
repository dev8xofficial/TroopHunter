...
import { motion, MotionProps, useMotionTemplate, useMotionValue, useTransform } from 'framer-motion';
...
export function Spotlight({ children, className, strength = 0.5, ...props }: SpotlightProps) {
  ...
  const animatedMaskImage = useTransform([mouseX, mouseY, spotlightOpacity], ([x, y, o]: [number, number, number]) => `radial-gradient(circle ${size} at ${x * 100}% ${y * 100}%, rgba(255 255 255 / ${o}), transparent 80%)`);
