          ...
          if (Array.isArray(work)) {
            work.map((item, index) => (
              <WorkGridRowPortrait key={`portrait-card-${index}`}>
                <WorkCard variant={item.variant} space={item.space} bgColor={item.bgColor} title={item.title} image={item.images[index]} video={item.video} path={item.path} openModal={openModal} />
              </WorkGridRowPortrait>
            ));
          } else {
            return (
              <WorkGridRowLandscape key={`landscape-${index}`}>
                <WorkCard variant={work.variant} space={work.space} bgColor={work.bgColor} title={work.title} image={work.images[0]} video={work.video} path={work.path} openModal={openModal} />
