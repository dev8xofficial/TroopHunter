...
export const UserUpdateVerifiedRequestSchema = UserSchema.pick({ verified: true });
...
export const ResetPasswordSchema = UserSchema.pick({ id: true, password: true }).extend({
  token: z.string().nonempty('User verification token must be attached in request.'),
  newPassword: z
    .string()
    .min(8, 'Password must be at least 8 characters long')
...
export const UserFetchByEmailRequestValidationMiddleware = validationMiddleware(UserFetchByEmailRequestSchema, 'params');
...
export const UserUpdateVerifiedRequestValidationMiddleware = validationMiddleware(UserUpdateVerifiedRequestSchema, 'body');
